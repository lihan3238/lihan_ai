...
for keyword in "docker-compose.cpa.yml" "docker-compose.cpa.ui.yml" "ssh -L 8317" "ops/sync-cpa-upstream-assets.sh" "/opt/lihan_ai_deploy/shared/data/cpa" "docker compose -p lihan_ai" "docker-compose.cloudflare-tunnel.yml" "ops/cpa-ui.sh" "ops/cpa-quota-snapshot.sh" "ops/cpa-quota-refresh-all.sh" "cpa-quota/home.html" "cpa-quota/widget.html" "quota-snapshot.json" "cpa-quota-static" "--no-deps" "logs-max-total-size-mb" "error-logs-max-files"; do
