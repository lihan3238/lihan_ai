...
assert_contains "$workflow" "find ops scripts tests"
assert_contains "$workflow" "tests/\\*.test.sh"
assert_contains "$workflow" "docker compose --env-file .env.example"
assert_contains "$workflow" "docker-compose.local-build.yml"
...
assert_contains "$workflow" "scripts/verify-repo.sh --skip-docker"
assert_not_contains "$workflow" "verify-repo.ps1"
assert_not_contains "$workflow" "pwsh"
assert_not_contains "$workflow" "docker-compose.kuma.ui.yml"
assert_not_contains "$workflow" "docker-compose.ops-dashboard.yml"
...
assert_not_contains "$workflow" "PROD_DEPLOY_"
assert_not_contains "$workflow" "docs/ai-dev"
assert_not_contains "$workflow" "ops/ai-dev-check.sh"
assert_not_contains "$workflow" "npm run e2e"
