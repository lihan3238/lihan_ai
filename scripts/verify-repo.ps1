...
Assert-File ".github/workflows/cd.yml"
...
Assert-File "ops/local-new-api-e2e.sh"
Assert-File "ops/pre-commit.sh"
...
Assert-File "tests/ci-cd-pipeline.test.sh"
Assert-File "tests/local-new-api-e2e.test.sh"
...
Assert-Contains "docs/browser-e2e-runbook.md" "ops/local-new-api-e2e\.sh" "local full E2E command"
...
Assert-Contains "ops/local-new-api-e2e.sh" "codex_e2e_admin" "local E2E admin default"
Assert-Contains "ops/local-new-api-e2e.sh" "localhost" "local E2E localhost guard"
Assert-Contains "ops/pre-commit.sh" "git diff --check" "pre-commit whitespace check"
...
Assert-Contains "docs/new-api-small-circle-launch-runbook.md" "v1.0.0-rc.5" "small circle official rc.5 policy"
Assert-Contains "docs/new-api-small-circle-launch-runbook.md" "DEPLOY_INCLUDE_LOCAL_NEW_API_BUILD=0" "small circle official image default"
...
Assert-Contains "docs/zh-CN/new-api-small-circle-launch-runbook.md" "v1.0.0-rc.5" "Chinese small circle official rc.5 policy"
Assert-Contains "docs/zh-CN/new-api-small-circle-launch-runbook.md" "DEPLOY_INCLUDE_LOCAL_NEW_API_BUILD=0" "Chinese small circle official image default"
...
Assert-NotContains ".github/workflows/ci.yml" "PROD_DEPLOY_" "CI must not read production deploy secrets"
Assert-NotContains ".github/workflows/ci.yml" "npm run e2e" "CI must not run browser E2E"
Assert-Contains ".github/workflows/cd.yml" "post-merge validation" "CD runs runner-only post-merge validation"
Assert-Contains ".github/workflows/cd.yml" "ops/dev-gate\.sh" "CD runs local repository gate on the runner"
Assert-Contains ".github/workflows/cd.yml" "Runner compose smoke" "CD starts a disposable compose smoke on the runner"
Assert-Contains ".github/workflows/cd.yml" "up -d postgres redis new-api" "CD starts local services on the runner"
Assert-Contains ".github/workflows/cd.yml" "127\.0\.0\.1:3100/api/status" "CD checks local New API status"
Assert-NotContains ".github/workflows/cd.yml" "PROD_DEPLOY_" "CD must not read production deploy secrets"
Assert-NotContains ".github/workflows/cd.yml" "deploy-release\.sh" "CD must not SSH deploy production"
