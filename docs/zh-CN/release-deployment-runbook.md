...
`build` 模式会追加 `docker-compose.local-build.yml`，从当前 release pin 住的 `vendor/new-api` 构建 `new-api`，其它服务仍使用拉取的镜像。它只适合内存足够的机器；小生产机不要在服务器上编译 New API 前端，应该在本地工作站或 CI 先构建并推送补丁镜像，然后用 pull 模式：
...
3. 让 GitHub Actions 只在自己的 runner 上跑 post-merge validation；它不能 SSH 到生产，也不能运行 release 命令。
4. 在可信的本机/operator shell 里，对 `DEPLOY_REF=main` 运行 `prepare`。
5. 运行 `smoke`；需要指定 dump 时传 `SMOKE_BACKUP_PATH`。
6. Smoke 通过后才运行 `promote`。
7. 验证 runtime、备份、New API 后台、测试 token、可选 CPA 路由和可选 tunnel 路由。

GitHub Actions 刻意不作为生产部署 actor。不要把生产 SSH key 或部署 secret 加到仓库 Actions secrets；服务器访问保留在 operator 机器上。
